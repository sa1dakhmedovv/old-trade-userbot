...
async def process_name(message: Message, state: FSMContext):
    ...
    if not name:
        return await message.answer("❗ Nom bo'sh bo'lmasligi kerak.")

    # session fayl mavjud emasligini tekshir
    session_file = os.path.join(SESSIONS_DIR, f"{name}.session")
    if os.path.exists(session_file):
        return await message.answer("❗ Bu nomdagi session allaqachon mavjud. Boshqa nom tanlang.")
...
async def process_code(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("🔐 2FA parol kerak.\nIltimos, Telegram hisobingizdagi ikki bosqichli parolni kiriting:")
...
async def process_password(message: Message, state: FSMContext):
    ...
    try:
        ...
        await client.disconnect()
        await message.answer(f"❌ 2FA parolda xato yoki boshqa xato:\n{e}")
        await state.clear()
